...
4. **Hardware Binding**: Your agreement is cryptographically signed and stored in `%LOCALAPPDATA%\DualAI Arena\encrypted-results\legal-agreement.enc` using Windows DPAPI, binding the license to your machine ID (`SHA256`).
...
2. **Loopback Isolation**: The backend server binds strictly to loopback IP `127.0.0.1`, rejecting any external local network (LAN) requests.
...
4. **Strict Memory Ceilings**: 128 MB max request body ceiling, 32 MB single-file limit, and 128 MB total workspace memory protection to prevent denial-of-service.
5. **CSRF & Owner Token Isolation**: All API endpoints enforce custom `X-Arena-Request` anti-forgery headers and per-session cryptographic owner tokens.
...
**A**: Press `Ctrl + Shift + A` anywhere in the application, or double-click the **`// DUALAI MID ARENA`** logo in the top-left corner. You can generate fresh PBKDF2 admin credentials or inspect encrypted store logs.
